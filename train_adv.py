...
from model.tbh_model import AdvModel as Model, step_train
...
def main():
    ...
    result_path = os.path.join(ROOT_PATH, 'result', conf.set_name + '_adv')
    ...
    data_iter = iter(data)
    opt_1 = tf.keras.optimizers.Adam(1e-4)
    opt_2 = tf.keras.optimizers.Adam(1e-4)
    writer = tf.summary.create_file_writer(summary_path)
    checkpoint = tf.train.Checkpoint(actor_opt=opt_1, critic_opt=opt_2, model=model)
    for i in range(conf.max_iter):
        batch_1 = next(data_iter)
        batch_2 = next(data_iter)
        with writer.as_default():

            loss = step_train(conf, batch_1, batch_2, model, opt_1, opt_2, i)
